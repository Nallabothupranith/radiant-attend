...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    const branchStats = branches.map((branch) => {
      ...
      const avgAttendance =
        branchStudents.reduce((acc, s) => acc + s.attendance, 0) /
        branchStudents.length;
      const highRisk = branchStudents.filter(
        (s) => s.riskLevel === "high"
      ).length;
      ...
      <div className="p-6 aurora-bg">
        ...
        >
          ...
          <p className="text-muted-foreground">
            Welcome to SchoolHub Administrative Portal
          </p>
        </motion.div>
